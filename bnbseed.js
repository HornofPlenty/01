...
contract BNBSeed {
	...
	function bonus(uint8 times) public payable {
		...
		for (uint256 p = 0; p < times; p++) {
			...
			uint index1 = uint(keccak256(abi.encodePacked(nonce, msg.sender, block.difficulty, index)))% totalSize;
			if(index1 > 50){
				success = success + 1;
			}
		}
	...
	function getUserInfo(address userAddress) public view returns(uint256 totalDeposit, uint256 totalWithdrawn, uint256 totalReferrals, uint256 yourboomplan, uint256 yourboompercent, uint256 yourbonuspercent) {
		return(getUserTotalDeposits(userAddress), getUserTotalWithdrawn(userAddress), getUserTotalReferrals(userAddress), getyourboomplan(userAddress), getyourboompercent(userAddress), getyourbonuspercent(userAddress));
	}
